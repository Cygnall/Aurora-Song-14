...
public sealed class ContrabandStatisticsSystem : EntitySystem
{
    ...
    private void OnDatabaseUiOpened(EntityUid uid, ContrabandDatabaseConsoleComponent component, BoundUIOpenedEvent args)
    {
        ...
        UpdateDatabaseTerminalUi(uid, component, statsComp, args.Actor);
    }
    ...
    private void UpdateDatabaseTerminalUis(EntityUid station, ContrabandStatisticsComponent statsComp)
    {
        ...
        while (query.MoveNext(out var uid, out var console))
        {
            ...
            // Update UI for each actor that has it open
            if (_uiSystem.TryGetOpenUis(uid, ContrabandDatabaseUiKey.Key, out var uis))
            {
                foreach (var actor in uis)
                {
                    UpdateDatabaseTerminalUi(uid, console, statsComp, actor);
                }
            }
        }
    }

    private void UpdateDatabaseTerminalUi(EntityUid uid, ContrabandDatabaseConsoleComponent component, ContrabandStatisticsComponent statsComp, EntityUid actor)
    {
        ...
        _uiSystem.SetUiState(uid, ContrabandDatabaseUiKey.Key, state, actor);
    }
}
